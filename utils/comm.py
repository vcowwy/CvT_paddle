...
def all_gather(data):
    ...
    storage = np.frombuffer(buffer)
    tensor = paddle.to_tensor(storage, dtype=paddle.int8)

    # obtain Tensor size of each rank
    local_size = paddle.to_tensor([tensor.numel()], dtype=paddle.int64)
    size_list = [paddle.to_tensor([0], dtype=paddle.int64)for _ in range(world_size)]
    ...
    tensor_list = []
    for _ in size_list:
        tensor_list.append(paddle.to_tensor(size=(max_size,), dtype=paddle.int8))
    if local_size != max_size:
        padding = paddle.to_tensor(size=(max_size - local_size,), dtype=paddle.int8)
