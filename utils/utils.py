...
def init_distributed(args):
    ...
    if args.distributed:
        ...
        paddle.set_device(args.local_rank)
        #paddle.init_process_group(backend='nccl', init_method='env://', timeout=timedelta(minutes=180))
...
def setup_cudnn(config):
    ...
    paddle.backends.cudnn.deterministic = config.CUDNN.DETERMINISTIC
"""
...
def summary_model_on_master(model, config, output_dir, copy):
    if comm.is_main_process():
        ...
        shutil.copy2(os.path.join(this_dir, '../models', config.MODEL.NAME +
            '.py'), output_dir)
        logging.info('=> {}'.format(model))
        try:
            num_params = count_parameters(model)
            logging.info('Trainable Model Total Parameter: \t%2.1fM' %
                num_params)
        except Exception:
        ...
        if config.MODEL_SUMMARY:
            try:
                ...
                df = tw.model_stats(model, (1, 3, config.TRAIN.IMAGE_SIZE[1
            ...
            try:
                ...
                macs, params = get_model_complexity_info(model, (3, config.
                    TRAIN.IMAGE_SIZE[1], config.TRAIN.IMAGE_SIZE[0]),
...
def resume_checkpoint(model, optimizer, config, output_dir, in_epoch):
    best_perf = 0.0
    begin_epoch_or_step = 0
    checkpoint = os.path.join(output_dir, 'checkpoint.pdiparams'
    ...
    if config.TRAIN.AUTO_RESUME and os.path.exists(checkpoint):
        ...
        checkpoint_dict = paddle.load(checkpoint)
        best_perf = checkpoint_dict['perf']
        begin_epoch_or_step = checkpoint_dict['epoch' if in_epoch else 'step']
        state_dict = checkpoint_dict['state_dict']
        model.load_state_dict(state_dict)
        optimizer.load_state_dict(checkpoint_dict['optimizer'])
        logging.info("=> {}: loaded checkpoint '{}' ({}: {})".format(comm.
            head, checkpoint, 'epoch' if in_epoch else 'step',
            begin_epoch_or_step))
    return best_perf, begin_epoch_or_step


def save_checkpoint_on_master(model, *, distributed, model_name, optimizer,
    output_dir, in_epoch, epoch_or_step, best_perf):
    if not comm.is_main_process():
        return
    states = model.module.state_dict() if distributed else model.state_dict()
    logging.info('=> saving checkpoint to {}'.format(output_dir))
    save_dict = {('epoch' if in_epoch else 'step'): epoch_or_step + 1,
        'model': model_name, 'state_dict': states, 'perf': best_perf,
        'optimizer': optimizer.state_dict()}
    try:
        paddle.save(save_dict, os.path.join(output_dir, 'checkpoint.pdiparams')
            )
    except Exception:
...
def save_model_on_master(model, distributed, out_dir, fname):
    ...
    try:
        ...
        paddle.save(model.module.state_dict() if distributed else model.state_dict(), fname_full)
    except Exception:
        logging.error('=> error when saving checkpoint!')
