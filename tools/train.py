...
def main():
    ...
    if comm.is_main_process():
        ...
        logging.info(config)
        logging.info('=> using {} CPUs'.format(args.num_gpus))
   ...
   # summary_model_on_master(model, config, final_output_dir, True)
    ...
    writer_dict = {
        'writer': LogWriter(logdir=tb_log_dir),
    ...
    scaler = paddle.amp.GradScaler(enable=config.AMP.ENABLED)
    ...
    for epoch in range(begin_epoch, config.TRAIN.END_EPOCH):
        ...
        if config.TRAIN.SAVE_ALL_MODELS and comm.is_main_process():
            save_model_on_master(
                model, args.distributed, final_output_dir, f'model_{epoch}.pdparams')

        logging.info('=> {} epoch end, duration : {:.2f}s'
                     .format(head,  time.time() - start))

    save_model_on_master(
        model, args.distributed, final_output_dir, 'final_state.pdparams')

    if config.SWA.ENABLED and comm.is_main_process():
        save_model_on_master(
            args.distributed, final_output_dir, 'swa_state.pdparams')
